...
\begin{enumerate}
    ...
    \item List covariates that we'd like to include (from protocols).  \textcolor{blue}{I am not really sure what makes a variable we would like to include (without any underlying understanding of the relationship between all the variables, why not include them all?).  At the very least, I think we should have data from the blood, urine, and pelvic samples and measures of adherence. It would also probably be useful to have data on which services they chose to receive:
    \begin{itemize}
    \item  Condoms
    \item  Used/unused study VR(s)
    \item  Provision of study VR(s)
    \item  Digital exam by clinician to check VR placement*
    \item  Protocol adherence
    \item  HIV/STI risk reduction
    \item  HIV pre- and post-test 
    \end{itemize} 
    Many of these variables are longitudinal, but that should be fine as this can be handled by the ltmle package.
    }
